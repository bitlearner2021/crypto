...
public class TransactionControllerTest {
    ...
    public void testFindExistingTransaction() throws Exception {
        ...
        when(transactionsRepository.getOne(1L)).thenReturn(transaction);

        mockMvc.perform(get("/rest/transactions/1"))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.amount", CoreMatchers.is(100000)))
                .andExpect(jsonPath("$.rawTransaction", CoreMatchers.is("rawTransaction")))
                .andExpect(jsonPath("$.links[*].href", Matchers.hasItem(Matchers.endsWith("/transactions/1"))));
    }
    ...
    public void testSignTransaction() throws Exception {
        ...
        mockMvc.perform(post("/rest/transactions/1")
                ...
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.amount", CoreMatchers.is(100000)))
                .andExpect(jsonPath("$.rawTransaction", CoreMatchers.is("signedTransaction")))
                .andExpect(jsonPath("$.links[*].href", hasItem(endsWith("/transactions/1"))));
    ...
    public void testFindNonExistingAddress() throws Exception {
        ...
        when(transactionsRepository.getOne(1L)).thenReturn(null);
